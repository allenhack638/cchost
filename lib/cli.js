...
Usage:
  ...
  cc list [--original] [--json]                    Show all profiles: login state, email, storage (shared/isolated)
...
Columns:
  ...
  Email     - account email (alias-aware; - if not yet logged in)
  Storage   - 'shared' if projects/ or sessions/ is linked to ~/.claude-shared,
              'isolated' otherwise

By default, the displayed email reflects any alias set via 'cc add'.
With --original, shows the real value from .claude.json.

With --json, emits an array of {name, dir, loggedIn, email, storage, alias}.
`,
...
function cmdList(rest) {
  ...
  const enriched = list.map((p) => {
    ...
    const storage = (projectsLink || sessionsLink) ? 'shared' : 'isolated';
    return {
      name: p.name,
      dir: p.dir,
      loggedIn: p.loggedIn,
