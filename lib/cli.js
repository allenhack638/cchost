...
Usage:
  ...
  cc list [--json]                                 Show all profiles: login state, email, shared-dir count (N/4), endpoint
  cc env <profile> [show] [...]                    Configure a custom API endpoint for a profile (wizard or flags)
...
With --json, emits an array of {name, dir, loggedIn, email, shared, endpoint}.
`,
  env: `cc env <profile> [show] [...]

Configure a custom, Anthropic-compatible API endpoint for a profile. This
routes the profile through a third-party provider (Moonshot/Kimi, OpenRouter,
vLLM, a corporate proxy, ...) instead of Anthropic's subscription OAuth.

Billing: an endpoint profile bills through the third-party provider, NOT your
Anthropic subscription.

Forms:
  cc env <profile>
      Interactive wizard. Prompts for every field; the token is not echoed.
      On an existing endpoint profile, the wizard runs in edit mode and
      prefills current values.

  cc env <profile> --base-url=URL --token=TOKEN
                   [--model=NAME] [--opus=NAME] [--sonnet=NAME]
                   [--haiku=NAME] [--subagent=NAME]
      Non-interactive. --base-url and --token are required when creating;
      when editing, only the flags you pass are changed.

  cc env <profile> show [--reveal]
      Print the endpoint configuration. The token is masked by default.
      --reveal prints the FULL API key — do not paste that output anywhere.

A profile is born either OAuth or endpoint and stays that way. cc env refuses
to run on a profile that already has OAuth credentials. To stop using an
endpoint, run 'cc remove <profile>' and recreate the profile.

The config lives at ~/.claude-profiles/<profile>/.cc-env.json with user-only
permissions. It is never migrated and never linked.

Examples:
  cc add kimi
  cc env kimi --base-url=https://api.moonshot.ai/anthropic --token=sk-... --model=kimi-k2.5
  cc env kimi show
  cc use kimi
`,
...
// Flag name -> config field for `cc env`.
...
async function cmdEnv(rest) {
  ...
  for (const a of rest) {
    if (a.startsWith('-')) {
      ...
      if (Object.prototype.hasOwnProperty.call(ENV_VALUE_FLAGS, flagName)) {
        if (eq === -1) {
          throw usageError(`Flag ${flagName} requires a value: ${flagName}=VALUE`);
        }
        provided[ENV_VALUE_FLAGS[flagName]] = a.slice(eq + 1);
        continue;
      }
  ...
  const [name, sub, ...extra] = positional;
  if (!name) throw usageError('Usage: cc env <profile> [show] [--base-url=URL --token=TOKEN ...]');
  ...
  if (sub === 'show') {
    if (Object.keys(provided).length > 0) {
      throw usageError('cc env <profile> show does not take configuration flags.');
    }
    return envLib.runShow(name, dir, { reveal });
  }
  if (sub) throw usageError(`Unknown subcommand for cc env: ${sub}`);
  if (reveal) {
    throw usageError('Unknown flag: --reveal (only valid with: cc env <profile> show)');
  }

  return envLib.runConfigure(name, dir, provided);
}
